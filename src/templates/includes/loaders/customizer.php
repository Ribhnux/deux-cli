...
if ( isset( ${{theme.slugfn}}_config['customizer']['control_types'] ) ) {
	foreach ( ${{theme.slugfn}}_config['customizer']['control_types'] as $name => $control ) {
		$customizer_control_path = {{{ themepath theme "'/includes/customizer/controls/class-wp-customize-' . $name . '-control.php'" }}};
...
if ( ! function_exists( '{{theme.slugfn}}_preview_init' ) ) :
	...
	function {{theme.slugfn}}_preview_init() {
		...
		if ( file_exists( $script_path ) ) {
			wp_enqueue_script( '{{theme.slug}}-customize-preview', {{{ themeuri theme '$js_dir . $customizer_preview' }}}, array( 'customize-preview' ), $theme_info->get( 'Version' ), true );
		}
	}
endif;
...
if ( ! function_exists( '{{theme.slugfn}}_customizer_control_scripts' ) ) :
	...
	function {{theme.slugfn}}_customizer_control_scripts() {
		...
		if ( file_exists( $style_path ) ) {
			wp_enqueue_style( '{{theme.slug}}-customizer', {{{ themeuri theme '$css_dir . $customizer_css' }}}, array(), $theme_version );
		}

		// Load customizer script.
		$script_path = get_theme_file_path( $js_dir . $customizer_js );
		if ( file_exists( $script_path ) ) {
			wp_enqueue_script( '{{theme.slug}}-customizer-control', {{{ themeuri theme '$js_dir . $customizer_js' }}}, array( 'jquery' ), $theme_version, true );
		}
	}
endif;
